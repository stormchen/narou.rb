...
$(function() {
  ...
  var table = t = $("#novel-list").DataTable({
    ...
    lengthMenu: [[20, 50, 100, 200, 500, -1], [20, 50, 100, 200, 500, "全部"]],
    ...
    language: {
      ...
      emptyTable: "尚未加入任何小說",
      zeroRecords: "找不到符合條件的小說",
      infoPostFix: "",
      info: "第 _PAGE_ 頁 / 共 _PAGES_ 頁",
      infoEmpty: "無相符資料",
      infoFiltered: "(從全體 _MAX_ 筆資料中篩選)",
      loadingRecords: "載入中...",
      processing: "處理中...",
      paginate: {
        first: "&laquo;",
  ...
  $("#action-view-all").on(click_event_name, function(e) {
    e.preventDefault();
    slideNavbar.slide();
    bootbox.confirm("確定要顯示所有欄位項目嗎？", function(result) {
      if (!result) return;
...
Mac 風格：<br>當選單超出畫面時，選單自動向上微調避免超出邊界
  ...
  $("#action-view-reset").on(click_event_name, function(e) {
    e.preventDefault();
    slideNavbar.slide();
    bootbox.confirm("將重設所有與顯示相關的設定，確定要繼續嗎？", function(result) {
      if (!result) return;
  ...
  $("#action-option-about").on(click_event_name, function(e) {
    ...
    $.get("/about", function(data) {
      bootbox.dialog({
        title: '<span class="glyphicon glyphicon-info-sign"></span> 關於 Narou.rb',
        message: data,
        backdrop: true
      });
    });
  ...
  $("#action-option-server-shutdown").on(click_event_name, function(e) {
    ...
    slideNavbar.slide();
    bootbox.dialog({
      title: '<span class="glyphicon glyphicon-off"></span> 關閉伺服器',
      message: "<p>即將關閉 Narou.rb WEB UI 伺服器。<br>" +
               "關閉後在重新啟動前將無法訪問 Web 介面。<br>" +
               "若要再次啟動，請在終端機中再次執行 <kbd>narou web</kbd>。</p>" +
  ...
  var register_badge_number_of_selecting = function(table) {
    table.on("changeselect", function() {
      ...
      if (selected_count > 0) {
        refreshTooltop(".button-update", "更新選取的小說");
        refreshTooltop(".button-send", "傳送選取的小說");
        enable_menu_item(".enable-selected");
      }
      else {
        refreshTooltop(".button-update", "更新所有小說");
        refreshTooltop(".button-send", "傳送有更新的小說");
