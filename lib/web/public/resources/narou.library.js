...
var Narou = (function() {
  ...
  $.extend(ContextMenu.prototype, {
        ...
        var diff_modal = bootbox.dialog({
          ...
          buttons: {
            clear: {
              ...
              callback: function() {
                bootbox.confirm("確定要清除所有差異記錄嗎？", function(result) {
                  if (result) {
    ...
    items: [
      { label: "――――――――(分隔線)", command: "divider" },
      { label: "小說轉換設定", command: "setting" },
      { label: "顯示文字差異", command: "diff" },
      { label: "編輯標籤", command: "edit_tag" },
      { label: "凍結或解除凍結", command: "freeze_toggle" },
      { label: "更新", command: "update" },
      { label: "強制更新（包含已凍結）", command: "update_force" },
      { label: "傳送至裝置", command: "send" },
      { label: "刪除", command: "remove" },
      { label: "轉換為電子書", command: "convert" },
      { label: "顯示調查日誌", command: "inspect" },
      { label: "開啟儲存資料夾", command: "folder" },
      { label: "建立備份", command: "backup" },
      { label: "強制重新下載", command: "download_force" },
      { label: "透過電子郵件傳送", command: "mail" },
      { label: "顯示作者留言", command: "author_comments" },
    ],
  ...
  $.extend(Action.prototype, {
        ...
        $.get("/partial/download_form")
          .done(function(html) {
            var download_modal = bootbox.dialog({
              title: "請輸入要下載的小說 URL 或 N代碼（可輸入多筆）",
              message: html,
              backdrop: true,
              buttons: {
                cancel: {
    ...
    _updateGeneralLastupHtmlRenderer: _.template(
      (function () {/*
<p>僅確認並更新除已凍結以外各小說的最新話發表日。<br>
支援 なろう API 的小說將使用 API 批次快速檢查；其他網站小說則會逐一檢查，所需時間較長。</p>
<ul><li>已被作者設為排除於搜尋之外的小說無法透過 API 檢查。</li>
<li>最新話發表日亦會在一般的「更新」操作中自動同步。</li>
<li>確認有更新的小說會自動標註 <code>modified</code> 標籤，執行更新後將自動清除。（即使僅有 Narou.rb 未追蹤的小說資訊變更，亦會被判定為有更新）</li>
<li>“小說實際修改至反映至 なろう API 平均約有 5 分鐘（最多 2 小時）的時間差。”<span class="text-muted">（引用自<a href="http://dev.syosetu.com/man/api/" target="_blank">官方網站</a>）</span></li></ul>
<p class="text-right">
<label><input type="checkbox" id="update-gl-narou" <% if (narou) { %>checked<% } %>> 使用 API 檢查 なろう 小說</label>
&nbsp;&nbsp;&nbsp;<label><input type="checkbox" id="update-gl-other" <% if (other) { %>checked<% } %>> 檢查其他網站的小說</label><br>
<label><input type="checkbox" id="update-gl-and-update-modified" <% if (updateModified) { %>checked<% } %>> 檢查完畢後立即更新帶有 modified 標籤的小說</label>
</p>
      */}).toString().replace(/(\n)/g, '').split('*')[1]
    ),
    ...
    _removeConfirmDialog: function(title, ids, callback) {
      ...
      message = '<div style="max-height:300px;overflow:auto"><ul>' + message + '</ul></div>' +
                ...
                '連同儲存資料夾與檔案一併刪除</label></div><div class="clearfix"></div>';
      bootbox.dialog({
        title: title,
        message: message,
        buttons: {
    ...
    remove: function() {
      var ids = this._getSelectedIds(arguments);
      if (ids.length === 0) return;
      this._removeConfirmDialog("確定要刪除選取的小說嗎？", ids, function(result, with_file) {
        if (!result) return;
    ...
    rebootDialog: function() {
      var self = this;
      bootbox.dialog({
        title: '<span class="glyphicon glyphicon-refresh"></span> 重新啟動',
        message: "<p>即將重新啟動 Narou.rb WEB UI 伺服器。<br>" +
                 "若已更新版本，重啟後將載入最新版本。</p>" +
                 "<p class=text-danger>若目前有正在更新或轉換中的小說將會中斷。<br>" +
                 "請確認工作是否已全部完成。</p>",
        backdrop: true,
        buttons: {
          danger: {
  ...
  $.extend(Tag.prototype, {
    ...
    _createEditorField: function(ids, callback) {
      ...
      $.post("/api/taginfo.json", { ids: ids }, function(taginfo) {
        ...
        var input = $('<div><input type="text" id="new-tag" placeholder="新增標籤" class="form-control"></div>' +
            '<div><small>（若要新增多個標籤，請以半形空格分隔）</small></div>');
        field.append(input);
