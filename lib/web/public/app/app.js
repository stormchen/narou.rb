...
(function () {
  ...
  const state = {
    ...
    currentCharacterNovelId: null, // 當前編輯角色名稱的小說 ID
  };
  ...
  const dom = {
    ...
    characterNamesModal: document.getElementById('characterNamesModal'),
    charModalNovelTitle: document.getElementById('charModalNovelTitle'),
    charNamesTableBody: document.getElementById('charNamesTableBody'),
    btnAddCharRow: document.getElementById('btnAddCharRow'),
    btnScanCharNames: document.getElementById('btnScanCharNames'),
    btnApplyCharFix: document.getElementById('btnApplyCharFix'),
    charScanResultsArea: document.getElementById('charScanResultsArea'),
    charScanResultsList: document.getElementById('charScanResultsList'),
    btnAddSelectedVariants: document.getElementById('btnAddSelectedVariants'),
    btnCloseCharModal: document.getElementById('btnCloseCharModal'),
    btnCancelCharModal: document.getElementById('btnCancelCharModal'),
    btnSaveCharModal: document.getElementById('btnSaveCharModal'),
    linkOpenLegacySetting: document.getElementById('linkOpenLegacySetting'),
        ...
        <div class="novel-card ${novel.frozen ? 'frozen' : ''}" data-id="${novel.id}">
          ...
          <div class="card-actions">
            <div class="card-actions-left">
              ...
              <button class="btn btn-sm btn-outline" data-action="characters" data-id="${novel.id}" title="管理角色名稱對照表與翻譯糾錯">角色</button>
      ...
      .map((novel) => {
        ...
        <tr data-id="${novel.id}">
          ...
          <td style="text-align: right;">
            ...
            <button class="btn btn-sm btn-outline" data-action="characters" data-id="${novel.id}" title="管理角色名稱對照表">角色</button>
  ...
  function bindNovelEvents(container) {
    ...
    container.querySelectorAll('[data-action]').forEach((btn) => {
      btn.addEventListener('click', (e) => {
        ...
        if (action === 'characters') openCharacterNamesModal(id);
  ...
  // =========================================================================
  // Character Names & Consistency Operations (角色對照表與翻譯糾錯)
  // =========================================================================
  ...
  async function openCharacterNamesModal(id) {
    state.currentCharacterNovelId = id;
    const novel = state.novels.find((n) => String(n.id) === String(id));
    const title = novel ? novel.title : `小說 ID: ${id}`;

    if (dom.charModalNovelTitle) dom.charModalNovelTitle.textContent = title;
    if (dom.linkOpenLegacySetting) dom.linkOpenLegacySetting.href = `/novels/${id}/setting#character-names-panel`;
    if (dom.charScanResultsArea) dom.charScanResultsArea.style.display = 'none';
    if (dom.charNamesTableBody) {
      dom.charNamesTableBody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: var(--color-text-muted); padding: 16px;">載入中...</td></tr>`;
    }

    dom.characterNamesModal.classList.add('open');

    try {
      const res = await fetch(`/api/novels/${id}/character_names`);
      if (res.ok) {
        const data = await res.json();
        const characters = data.characters || [];
        renderCharacterTableRows(characters);
      } else {
        showToast('無法取得角色名稱設定', 'error');
        renderCharacterTableRows([]);
      }
    } catch (e) {
      console.error(e);
      showToast('載入角色名稱失敗', 'error');
      renderCharacterTableRows([]);
    }
  }

  function renderCharacterTableRows(characters) {
    if (!dom.charNamesTableBody) return;
    dom.charNamesTableBody.innerHTML = '';

    const list = (characters && characters.length > 0)
      ? characters
      : [{ original: '', translation: '', alternatives: [] }];

    list.forEach((char) => {
      addCharacterRow(char.original || '', char.translation || '', Array.isArray(char.alternatives) ? char.alternatives.join(', ') : (char.alternatives || ''));
    });
  }

  function addCharacterRow(original = '', translation = '', alternatives = '') {
    if (!dom.charNamesTableBody) return;
    const tr = document.createElement('tr');
    tr.className = 'char-row';
    tr.innerHTML = `
      <td><input type="text" class="form-control char-original" value="${escapeHtml(original)}" placeholder="例：ルカ" style="padding: 6px 10px; font-size: 13px;"></td>
      <td><input type="text" class="form-control char-translation" value="${escapeHtml(translation)}" placeholder="例：露卡" style="padding: 6px 10px; font-size: 13px;"></td>
      <td><input type="text" class="form-control char-alternatives" value="${escapeHtml(alternatives)}" placeholder="例：路卡, 盧卡" style="padding: 6px 10px; font-size: 13px;"></td>
      <td style="text-align: center;">
        <button type="button" class="btn-icon btn-sm btn-remove-char" title="刪除此角色" style="border: none; color: var(--color-danger); margin: 0 auto;">
    ...
    `;
    tr.querySelector('.btn-remove-char').addEventListener('click', () => {
      const rows = dom.charNamesTableBody.querySelectorAll('.char-row');
      if (rows.length <= 1) {
        addCharacterRow();
      }
      tr.remove();
    });
    dom.charNamesTableBody.appendChild(tr);
  }

  function getCharacterFormData() {
    const rows = dom.charNamesTableBody ? dom.charNamesTableBody.querySelectorAll('.char-row') : [];
    const list = [];
    rows.forEach((row) => {
      const original = (row.querySelector('.char-original')?.value || '').trim();
      const translation = (row.querySelector('.char-translation')?.value || '').trim();
      const alternativesStr = (row.querySelector('.char-alternatives')?.value || '').trim();
      if (original || translation) {
        const alternatives = alternativesStr ? alternativesStr.split(/[,、\s]+/).map(s => s.trim()).filter(Boolean) : [];
        list.push({ original, translation, alternatives });
      }
    });
    return list;
  }

  async function saveCharacterNames() {
    if (!state.currentCharacterNovelId) return;
    const id = state.currentCharacterNovelId;
    const list = getCharacterFormData();
    const btn = dom.btnSaveCharModal;
    ...
    try {
      ...
      if (res.ok) {
        showToast('角色名稱對照表儲存成功！', 'success');
        dom.characterNamesModal.classList.remove('open');
      } else {
      ...
      if (btn) { btn.disabled = false; btn.textContent = '儲存對照表'; }
    }
  }

  async function scanCharacterInconsistencies() {
    if (!state.currentCharacterNovelId) return;
    const id = state.currentCharacterNovelId;
    const list = getCharacterFormData();
    if (list.length === 0) {
      showToast('請先填入至少一個角色的日文原名與正確譯名', 'warning');
      return;
    }

    const btn = dom.btnScanCharNames;
    if (btn) { btn.disabled = true; btn.textContent = '掃描分析中...'; }

    try {
      const res = await fetch(`/novels/${id}/scan_names`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ character_names: list }),
      });
      if (res.ok) {
        const data = await res.json();
        ...
        showToast('掃描失敗，請檢查快取目錄', 'error');
      }
    } catch (e) {
      console.error(e);
      showToast('掃描請求失敗', 'error');
    } finally {
      if (btn) { btn.disabled = false; btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg><span>掃描快取不一致</span>`; }
    }
  }

  function renderScanResults(results) {
    if (!dom.charScanResultsArea || !dom.charScanResultsList) return;
    dom.charScanResultsList.innerHTML = '';

    const targets = Object.keys(results);
    if (targets.length === 0) {
      dom.charScanResultsList.innerHTML = `<div style="color: var(--color-text-muted);">🎉 太棒了！在目前已翻譯的快取中，未發現與對照表不一致的疑似譯名變體。</div>`;
      if (dom.btnAddSelectedVariants) dom.btnAddSelectedVariants.style.display = 'none';
      dom.charScanResultsArea.style.display = 'block';
      return;
    }

    targets.forEach((target) => {
      const variants = results[target] || [];
      const groupDiv = document.createElement('div');
      groupDiv.style.padding = '8px 12px';
      groupDiv.style.background = 'var(--color-bg-card, #fff)';
      groupDiv.style.borderRadius = 'var(--radius-sm)';
      groupDiv.style.border = '1px solid var(--color-border)';

      let html = `<div style="font-weight: 600; margin-bottom: 6px;">🎯 正確譯名「<strong>${escapeHtml(target)}</strong>」的疑似變體：</div>`;
      html += `<div style="display: flex; gap: 12px; flex-wrap: wrap;">`;
      variants.forEach((v) => {
        html += `
          <label style="display: inline-flex; align-items: center; gap: 6px; cursor: pointer; background: var(--color-bg); padding: 4px 8px; border-radius: 4px; font-size: 12px;">
            <input type="checkbox" class="scan-variant-checkbox" data-target="${escapeHtml(target)}" data-variant="${escapeHtml(v.variant)}" checked style="accent-color: var(--color-primary);">
            <span>${escapeHtml(v.variant)} <span style="opacity: 0.6;">(出現 ${v.count} 次)</span></span>
          </label>
        `;
      });
      html += `</div>`;
      groupDiv.innerHTML = html;
      dom.charScanResultsList.appendChild(groupDiv);
    });

    if (dom.btnAddSelectedVariants) dom.btnAddSelectedVariants.style.display = 'inline-flex';
    dom.charScanResultsArea.style.display = 'block';
  }

  function addSelectedVariantsToInputs() {
    if (!dom.charScanResultsArea) return;
    const checkboxes = dom.charScanResultsArea.querySelectorAll('.scan-variant-checkbox:checked');
    if (checkboxes.length === 0) {
      showToast('未選取任何變體', 'warning');
      return;
    }
    ...
    checkboxes.forEach((cb) => {
      ...
      const rows = dom.charNamesTableBody.querySelectorAll('.char-row');
      rows.forEach((row) => {
        const transInput = row.querySelector('.char-translation');
        if (transInput && transInput.value.trim() === target) {
          const altInput = row.querySelector('.char-alternatives');
          if (altInput) {
            const currentVals = altInput.value.split(/[,、\s]+/).map(s => s.trim()).filter(Boolean);
            if (!currentVals.includes(variant)) {
              currentVals.push(variant);
              altInput.value = currentVals.join(', ');
              addedCount++;
            }
          }
        }
    ...
    showToast(`已成功加入 ${addedCount} 個變體至對照表，請記得點擊「儲存對照表」！`, 'success');
    dom.charScanResultsArea.style.display = 'none';
  }

  async function applyCharacterFixToCache() {
    if (!state.currentCharacterNovelId) return;
    const id = state.currentCharacterNovelId;

    const list = getCharacterFormData();
    const hasAnyAlternatives = list.some((c) => c.alternatives && c.alternatives.length > 0);
    if (!hasAnyAlternatives) {
      showToast('請先填寫至少一個角色的「已知錯誤變體」，或點擊「掃描快取不一致」來找出變體！', 'warning');
      return;
    }

    if (!confirm("⚠️ 此操作將自動儲存當前角色對照表，並對所有快取章節進行批次替換修正。\n\n此修正操作無法還原（除非重新翻譯），是否確定執行？")) {
      return;
    }

    const btn = dom.btnApplyCharFix;
    if (btn) { btn.disabled = true; btn.textContent = '批次修復中...'; }

    try {
      const res = await fetch(`/novels/${id}/apply_name_fix`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ characters: list }),
      });
      if (res.ok) {
        const data = await res.json();
        if (data.modified > 0) {
          showToast(`🎉 修復完成！已自動儲存對照表並更新了 ${data.modified} 個快取檔案！`, 'success');
        } else {
          showToast(`對照表已儲存，但在既有快取中未發現匹配的錯誤變體字串（更新了 0 個檔案）。`, 'info');
        }
      } else {
        showToast('修復執行失敗', 'error');
      }
    } catch (e) {
      console.error(e);
      showToast('執行修復異常', 'error');
    } finally {
      if (btn) { btn.disabled = false; btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg><span>套用修正到既有快取</span>`; }
    }
  }
  ...
  function bindEvents() {
    ...
    // Character Names Modal Actions
    if (dom.btnAddCharRow) dom.btnAddCharRow.addEventListener('click', () => addCharacterRow());
    if (dom.btnCloseCharModal) dom.btnCloseCharModal.addEventListener('click', () => dom.characterNamesModal.classList.remove('open'));
    if (dom.btnCancelCharModal) dom.btnCancelCharModal.addEventListener('click', () => dom.characterNamesModal.classList.remove('open'));
    if (dom.btnSaveCharModal) dom.btnSaveCharModal.addEventListener('click', saveCharacterNames);
    if (dom.btnScanCharNames) dom.btnScanCharNames.addEventListener('click', scanCharacterInconsistencies);
    if (dom.btnAddSelectedVariants) dom.btnAddSelectedVariants.addEventListener('click', addSelectedVariantsToInputs);
    if (dom.btnApplyCharFix) dom.btnApplyCharFix.addEventListener('click', applyCharacterFixToCache);
